...
const complaints: Complaint[] = [
  { id: 1, label: "Ear Discharge / Pain / Hearing Loss" },
  { id: 2, label: "Sinusitis / Epistaxis / Nasal Congestion" },
  { id: 3, label: "Sore Throat / Hoarseness / Neck Mass" },
  { id: 4, label: "Tonsillitis / Adenoiditis" },
];

export default function CcEnt() {
  const [selected, setSelected] = useState<number | null>(null);

  const handleProceed = (): void => {
    const chosen = complaints.find((c) => c.id === selected);
    console.log("ENT complaint selected:", chosen?.label);
  };
    ...
    <div className="hidden">
      ...
      <div className="min-h-screen flex flex-col gap-6 items-center justify-center bg-cyan-950 py-12">
        ...
        <div className="flex flex-col text-center">
          ...
          <h1 className="text-5xl font-bold text-white">ENT</h1>
          <p className="text-xl font-serif text-white mt-1">For ear, nose, and throat concerns</p>
        </div>
        ...
        <div className="flex flex-row gap-5 bg-cyan-900 px-10 py-8 rounded-xl w-[70rem]">
          ...
          <div className="flex flex-col flex-1 gap-4">
            ...
            <div className="pt-2 border-t border-dashed border-cyan-700">
              <button
                type="button"
                onClick={() => console.log("Not listed – ENT")}
